...
/* Filter IDs */
enum H5Z_FILTER_ERROR       = (-1);	/*no filter			*/
enum H5Z_FILTER_NONE        = 0;	/*reserved indefinitely		*/
enum H5Z_FILTER_DEFLATE	    = 1;	/*deflation like gzip	     	*/
enum H5Z_FILTER_SHUFFLE     = 2;       /*shuffle the data              */
enum H5Z_FILTER_FLETCHER32  = 3;       /*fletcher32 checksum of EDC    */
enum H5Z_FILTER_SZIP        = 4;       /*szip compression              */
enum H5Z_FILTER_NBIT        = 5;       /*nbit compression              */
enum H5Z_FILTER_SCALEOFFSET = 6;       /*scale+offset compression      */
enum H5Z_FILTER_RESERVED    = 256;	/*filter ids below this value are reserved for library use */

enum H5Z_FILTER_MAX	    = 65535;	/*maximum filter id		*/

/* General macros */
enum H5Z_FILTER_ALL	    = 0;	/* Symbol to remove all filters in H5Premove_filter */
